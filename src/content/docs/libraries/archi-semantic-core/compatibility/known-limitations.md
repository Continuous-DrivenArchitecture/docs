...
description: Honest boundaries â€” what this package cannot do yet or will never do.
---
...
- **Label Expression reference-prefix forms** â€” `${parent{...}}`,
...
- **`${specialization}` and `${viewpoint}`** placeholders â€” likewise left
  unresolved; the parser does not yet capture those attributes on visual
  objects.
- **ArchiMate Model Exchange files** â€” by design, never parsed nor
  generated (see [Compatibility philosophy](/libraries/archi-semantic-core/compatibility/philosophy/)).
- **Sketch/Canvas semantics** â€” preserved verbatim where encountered, but
  not given semantic treatment (no element/relationship semantics to
  provide).
- **Image binaries** â€” never extracted; `imagePath`-style references are
  exposed as pointers into the archive.

## Not provided

- **Mutation or serialization** â€” there is no way to write an `ArchiModel`
  back to `.archimate`, and no editing API. The package reads; consumers
  build the write path if they need one.
- **Rendering, layout, diagram routing, image rendering** â€” out of scope by
  design.
- **Quality rules** â€” `validateArchiModel` checks structural integrity only.
  Lint-style rules are a separate layer.
- **Browser zip-handling** â€” the `/archive` subpath is Node-only
  (`node:zlib`); the root entrypoint has no Node imports and stays
  bundler-safe.
- **Id lookup helpers, query language, graph storage** â€” flat arrays +
...
- The parser does not fabricate defaults that Archi does not store â€” except
