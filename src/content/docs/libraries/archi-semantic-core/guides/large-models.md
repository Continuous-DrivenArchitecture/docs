...
Nothing here is new API â€” this page documents the properties you can rely on.

## Flat collections, no hidden copying

Every collection (`elements`, `relationships`, `diagramObjects`, â€¦) is a
...
`inflateRawSync` **chunk by chunk** â€” the full entry is never held in memory
...
size, and the parsing step's cost proportional to the model's content â€” the
...
arrays â€” no re-parse of XML is ever triggered by validation.

## Practical scaling notes

- **Lookups**: avoid `collection.find()` inside loops over other
  collections â€” O(nÂ·m). Build `Map` indexes once
...
representation itself â€” at which point the right move is a streaming or
